...
namespace MPC {
    ...
    MPC::MPC()
    {
        ...
        std::cout << sizeof(ACADO) << " vs " << sizeof(acadoVariables) << std::endl;
    }
    ...
    {
        ExtractWindowTrajectory(trajectory, currentTrajectory_, position, velocity, q, 1.1*HorizonLength*ACADO_TS*desiredVelocity_, 0);
    ...
    void MPC::ExtractWindowTrajectory(Trajectory& trajectory, Trajectory& extractedWindowTrajectory, const Eigen::Vector2d& position, const Eigen::Vector2d& velocity, const boost::math::quaternion<double>& q, double ExtractionDistance, unsigned int OrientationSelection)
    {
        ...
        else if (OrientationSelection == 1) // robot yaw (heading)
            WindowOrientation_ = extractHeading(q);
        else if (OrientationSelection == 2) // velocity direction
    ...
    void MPC::setCurrentState(const Eigen::Vector2d& position, const Eigen::Vector2d& velocity, const boost::math::quaternion<double>& q)
    {
        ...
        Eigen::Matrix2d R_window = Eigen::Rotation2Dd(WindowOrientation_).toRotationMatrix();
        ...
        ACADO.x0.x = position[0] - windowPathOrigin_[0];
        ACADO.x0.y = position[1] - windowPathOrigin_[1];
    ...
    MPC::state_t MPC::getHorizonState(unsigned int horizonIndex)
    {
        ...
        Eigen::Vector2d InertialPosition = Eigen::Vector2d(ACADO.x[horizonIndex].x, ACADO.x[horizonIndex].y) + windowPathOrigin_;
